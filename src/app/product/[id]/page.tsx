      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        >
          <div className="relative aspect-square rounded-3xl overflow-hidden shadow-2xl">
            ...
            />
            {product.badge && (
              <span className="absolute top-5 left-5 bg-brand-pink text-white font-bold px-4 py-2 rounded-full text-sm">
                {product.badge}
              </span>
            )}
          </div>
        ...
        >
          <span className="text-brand-pink text-sm font-semibold uppercase tracking-widest mb-3">
            {product.category}
          </span>
          <h1 className="font-display font-bold text-3xl md:text-4xl text-brand-plum dark:text-white mb-4">
            {product.name}
          </h1>
          ...
          <div className="flex items-center gap-2 mb-5">
            ...
            <span className="text-brand-charcoal/50 dark:text-white/50 text-sm">
          ...
          <div className="flex items-baseline gap-3 mb-6">
            <span className="font-bold text-4xl text-brand-plum dark:text-white">
              {formatPrice(product.price)}
            </span>
            {product.originalPrice && (
              <span className="text-brand-charcoal/40 dark:text-white/40 text-xl line-through">
          ...
          {/* Description */}
          <p className="text-brand-charcoal/70 dark:text-white/70 leading-relaxed mb-8 text-base">
            {product.longDescription}
          </p>

          {/* Ingredients */}
          <div className="mb-8">
            <h3 className="font-bold text-brand-plum dark:text-white mb-3">
              Ingredients
            </h3>
            <div className="flex flex-wrap gap-2">
              {product.ingredients.map((ing) => (
                <span
                  key={ing}
                  className="bg-brand-pink/10 dark:bg-brand-pink/20 text-brand-pink text-xs font-medium px-3 py-1.5 rounded-full"
                >
                  {ing}
                </span>
              ))}
          ...
          <div className="flex items-center gap-4 mt-auto">
            <div className="flex items-center gap-2 bg-brand-cream dark:bg-white/10 rounded-2xl px-4 py-3">
              <button
                ...
                className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-brand-pink hover:text-white transition-all text-brand-plum dark:text-white"
              >
                <Minus size={16} />
              </button>
              <span className="w-8 text-center font-bold text-brand-plum dark:text-white text-lg">
                {qty}
              </span>
              <button
            ...
            <motion.button
              ...
              className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl font-bold text-base transition-all duration-300 ${
                added
                  ...
                  : "bg-brand-pink text-white hover:bg-brand-plum shadow-lg shadow-brand-pink/30"
              }`}
            >
              {added ? (
                <>
          ...
          {added && (
            ...
            >
              <Link href="/cart" className="text-brand-pink font-semibold text-sm hover:underline">
                View Cart & Checkout →
              </Link>
            </motion.div>
          )}
