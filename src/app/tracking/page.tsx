...
import { buildWhatsAppMessage, WHATSAPP_NUMBERS, type Order } from "@/lib/orders";
...
const steps: Step[] = [
  { id: 1, icon: Package, label: "Order Received", desc: "We've received your order and it's being confirmed.", time: "Just now" },
  { id: 2, icon: ChefHat, label: "Preparing", desc: "Our chefs are crafting your meal with fresh ingredients.", time: "5–10 min" },
  { id: 3, icon: Bike, label: "Out for Delivery", desc: "Your order is on its way! Track your rider in real-time.", time: "15–25 min" },
  { id: 4, icon: Home, label: "Delivered", desc: "Enjoy your meal! Don't forget to leave a review.", time: "~40 min" },
];
...
export default function TrackingPage() {
  ...
  const orderId = order?.id ?? "DRL-" + Math.random().toString(36).substring(2, 8).toUpperCase();
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      >
        ...
        <p className="text-brand-charcoal/60 dark:text-white/60 text-lg">
          Thank you for ordering from Darla&apos;s Foods.
        </p>
      </motion.div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-green-700 dark:text-green-400 text-sm mb-4">
                {waSent
                  ? "Your order details have been sent. We'll confirm shortly!"
                  : "Tap a button below to send your order details directly to Darla's Foods on WhatsApp so we can confirm it quickly."}
              </p>
              {!waSent && (
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
          <div>
            ...
            <p className="font-bold text-brand-plum dark:text-white text-lg font-mono">{orderId}</p>
          </div>
          <div>
            ...
            <p className="font-semibold text-brand-charcoal dark:text-white text-sm flex items-center justify-center gap-1">
              ...
              {order ? `${order.delivery.address}, ${order.delivery.city}` : "Your saved address"}
            </p>
          </div>
          <div>
            <p className="text-brand-charcoal/50 dark:text-white/50 text-xs uppercase tracking-wider mb-1">Estimated Arrival</p>
            ...
            return (
              ...
              >
                ...
                <div className={`flex-1 bg-white dark:bg-brand-plum/30 rounded-2xl p-5 border transition-all duration-500 ${
                  isActive ? "border-brand-pink shadow-md" : isCompleted ? "border-green-200 dark:border-green-900/50 opacity-80" : "border-brand-pink/10 opacity-50"
                }`}>
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <span className="text-xs text-brand-charcoal/40 dark:text-white/40 shrink-0 ml-3">{step.time}</span>
                  </div>
